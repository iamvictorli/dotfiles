...
-- vim.opt.writebackup = false -- if a file is being editted by another program (or was written to file when editting with another program), it is not allowed to be editted
...
vim.keymap.set("n", "sv", ":vsp<CR>", { desc = "[S]plit [V]ertically window" })
vim.keymap.set("n", "sx", "<C-w>c", { desc = "[S]plit [C]lose window" })

-- Window Resize
-- vim.keymap.set("n", "<leader>h", ":vertical resize -2<CR>", { desc = "decrease window width" })
-- vim.keymap.set("n", "<leader>j", ":resize +2<CR>", { desc = "increase window height" })
...
-- Yanks
vim.keymap.set("n", "Y", "y$", { desc = "yanks the rest of the line" })
vim.keymap.set({ "n", "v" }, "<leader>y", [["+y]], { desc = "[Y]anks to system clipboard" })

vim.keymap.set("n", "gV", "`[v`]", { desc = "highlights last inserted text" })

vim.keymap.set("n", "J", "mzJ`z", { desc = "removes and appends next line to current cursor line" })

vim.keymap.set("n", "x", '"_x', { desc = "deleting a single character without copying to register" })
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			require("telescope").setup({
				...
				-- defaults = {
				--   mappings = {
				--     i = { ['<c-enter>'] = 'to_fuzzy_refine', ["<C-q>"] = actions.send_selected_to_qflist + actions.open_qflist, },
				--   },
				-- },
				-- pickers = {}
				extensions = {
	...
	{ -- Autoformat
		...
		opts = {
			...
			format_on_save = function(bufnr)
				-- Only use ESLint’s --fix for JS/TS; fall back elsewhere
				local ft = vim.bo[bufnr].filetype
				-- specify which filetypes get ESLint
				local eslint_fmts = {
					javascript = true,
			...
			formatters_by_ft = {
				lua = { "stylua" },
				-- for JS/TS, use eslint_d with --fix
				javascript = { "eslint_d" },
